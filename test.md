...
2. [Step 1: Start with a Power BI-Optimized Semantic Model](#step-1-start-with-a-power-bi-optimized-semantic-model)
3. [Step 2: Publish the Semantic Model](#step-2-publish-the-semantic-model)
4. [Step 3: Configure in Power BI Service](#step-3-configure-in-power-bi-service)
5. [Step 4: Select Fields to Expose to AI](#step-4-select-fields-to-expose-to-ai)
6. [Step 5: Add AI Instructions](#step-5-add-ai-instructions)
7. [Step 6: Add Verified Answers](#step-6-add-verified-answers)
8. [Step 7: Add Descriptions to All Fields](#step-7-add-descriptions-to-all-fields)
...
| **Power BI** | Pro or Premium Per User (PPU) for semantic model development |
| **M365 Copilot** | Required for accessing agent via M365 Copilot, Excel Copilot, and Teams |
| **Copilot Studio** | Required for creating orchestrator agents and publishing to Teams |

### Tenant Settings (Admin Portal)

The following settings must be enabled by a Fabric/Power BI administrator:

| Setting | Location | Purpose |
|---------|----------|---------|
| **Fabric data agent** | Tenant settings → Copilot and Azure OpenAI Service | Enables Data Agent creation |
| **Cross-geo processing for AI** | Tenant settings → Copilot and Azure OpenAI Service | Required for AI processing |
| **Cross-geo storing for AI** | Tenant settings → Copilot and Azure OpenAI Service | Required for AI data storage |
| **Allow XMLA endpoints** | Tenant settings → Integration settings | Required for semantic model data sources |

### Workspace Configuration

| Setting | Value | Purpose |
|---------|-------|---------|
| **License Mode** | Fabric capacity or Premium | Required for Data Agent |
| **Semantic model storage format** | Large | Recommended for AI workloads |

### User Permissions

| Role | Minimum Permission |
|------|-------------------|
| **Agent Creator** | Workspace Contributor or higher |
| **Agent Consumer** | Read access to Data Agent + Read access to underlying data sources |
| **Semantic Model** | Build permission for interactive querying |

### Tools

| Tool | Purpose |
|------|---------|
| **Power BI Desktop** | Semantic model development, star schema validation |
| **Power BI Service** | AI prep configuration, publishing, agent creation, testing |
| **Copilot Studio** | Orchestrator agent creation, Teams publishing |
| **Microsoft Teams** | End-user consumption |
...
After verifying the star schema structure, publish the semantic model to Power BI Service. We publish **before** configuring AI settings because:

- **Real-time collaboration** — AI configurations in Service are immediately available to all users
- **No republish required** — Update AI settings without returning to Desktop
- **Direct Lake support** — Service supports Direct Lake models that cannot be opened in Desktop
- **Single source of truth** — Avoids sync conflicts between Desktop and Service versions
- **Faster iteration** — Test and refine AI settings directly where Copilot runs

---

### 2.1 Publish from Power BI Desktop
...
### 2.2 Select Destination Workspace
...
| **Semantic Model** | The underlying data model (AI configurations added in Service) |
...
- The semantic model is ready for AI configuration in Service

---

## Step 3: Configure in Power BI Service

### Overview

After publishing, configure the semantic model in Power BI Service for AI features. This includes enabling large model storage and accessing the Prep data for AI panel where all AI configurations will be done.

---

### 3.1 Navigate to the Workspace
...
| Better Decisions AR Analytics | Semantic model | The underlying data model |

---

### 3.2 Open the Semantic Model
...
### 3.4 Enable Large Model Storage (If Required)
...
### 3.5 Tenant Settings (Administrator)
...
- Prep data for AI panel is accessible
- Tenant settings are enabled (if you have admin access)

The semantic model is now ready for AI configuration.

---

## Step 4: Select Fields to Expose to AI

### Overview

The **Prep data for AI** feature in Power BI Service allows you to configure which parts of your semantic model the AI can access. This step focuses on selecting fields — limiting what Copilot can see to improve response accuracy.

---

### 4.1 Open Simplify the Data Schema

From the Prep data for AI panel (opened in Step 3), click **Simplify the data schema** from the left sidebar.
...
### 4.2 Field Selection Interface

The field selection interface displays all tables in your model:
...
### 4.3 Field Selection Guidelines
...
### 4.5 Verify Field Selection

After applying, confirm the field selection is saved:
...
- Only relevant tables and columns are exposed to AI
- Duplicate and technical fields are deselected
- Changes are saved via the Apply button
- The field selection panel shows correct checkmarks
...
AI Instructions provide context to help Copilot understand your data, business terminology, and how to respond to queries. Adding instructions in the semantic model ensures consistency across all AI experiences (Copilot, Data Agent, M365).

---

### 5.1 Open AI Instructions

1. In the Prep data for AI panel, click **Add AI instructions**
...
### 5.5 Verify AI Instructions

After applying, confirm the instructions are saved:
...
- Instructions are saved in the Prep data for AI panel
- All key business terminology is documented
- Response guidelines are included
- Instructions will apply to all Copilot experiences using this model

---

## Step 6: Add Verified Answers

### Overview

Verified answers are pre-approved visual responses linked to specific questions. When users ask matching questions, Copilot returns the verified visual instead of generating a new response — ensuring consistent, reliable answers.

---

### 6.1 Open Verified Answers

1. In the Prep data for AI panel, click **Verified answers**
...
### 6.2 Understanding Verified Answers
...
### 6.3 Create a Verified Answer from a Visual

1. Open the **Report** in Power BI Service (not the semantic model)
...
### 6.4 Add Phrases with Copilot Suggestions
...
### 6.6 Best Practices for Verified Answers
...
| **Test phrases in Copilot chat** before finalizing | Validates that the verified answer triggers correctly |

---

### 6.7 Recommended Verified Answers for AR Model
...
### 6.8 Verify Verified Answers

After creating verified answers, confirm they appear in the panel:
...
- Verified answers are immediately available in Copilot

---

## Step 7: Add Descriptions to All Fields
...
### 7.1 Open Semantic Model

From the semantic model details page, click **Open semantic model** in the toolbar.

<img width="1567" height="677" alt="Open semantic model" src="https://github.com/user-attachments/assets/open-semantic-model-service" />

**[NEEDS REPLACEMENT: Use uploaded image 1 - 1769179303305_image.png]**

---

### 7.2 Switch to Editing Mode

The semantic model opens in **Viewing** mode by default. To add descriptions:

1. Click the **Viewing** dropdown in the top-right corner
2. Select **Editing** to enable changes

<img width="1567" height="741" alt="Switch to Editing mode" src="https://github.com/user-attachments/assets/switch-to-editing-mode" />

**[NEEDS REPLACEMENT: Use uploaded image 2 - 1769179306635_image.png]**

> **Note:** The yellow banner "You are in Viewing mode and changes will not be saved" disappears once you switch to Editing mode.

---

### 7.3 Add Descriptions in Properties Pane

1. Select a table or column in the model diagram
2. The **Properties** pane appears on the right side
3. Enter a description in the **Description** field
4. Changes save automatically

<img width="1538" height="735" alt="Add description in Properties pane" src="https://github.com/user-attachments/assets/add-description-properties" />

**[NEEDS REPLACEMENT: Use uploaded image 3 - 1769179340172_image.png]**
...
### 7.4 Description Best Practices
...
### 7.6 Column Descriptions — dim_Customers AR
...
### 7.7 Column Descriptions — fact_AR Invoices

| Column | Description |
|--------|-------------|
| **Invoice Number** | Unique invoice identifier from billing system. Primary reference for invoice lookup. |
| **Invoice Date** | Date the invoice was issued to the customer. Used for aging calculations. |
| **Due Date** | Date payment is expected based on payment terms. Invoices past this date are overdue. |
| **Invoice Amount** | Original billed amount in USD before any payments or adjustments. |
| **Open Balance** | Remaining unpaid amount on the invoice in USD. Decreases as payments are received. |
| **Payment Date** | Date the most recent payment was received for this invoice. Null if unpaid. |
| **Days Past Due** | Number of days the invoice is overdue (current date minus due date). Zero or negative if not overdue. |

---

### 7.8 Column Descriptions — dim_Aging Bucket

| Column | Description |
|--------|-------------|
| **Aging Bucket Key** | Surrogate key for the aging period. Used for joins to fact table. |
| **Aging Period Name** | Display name for the aging period (e.g., "Current", "1-30 Days", "31-60 Days"). |
| **Aging Period Order** | Sort order for aging periods. Ensures correct sequence in reports (Current first, 180+ last). |
| **Is Overdue** | Boolean indicating if this bucket represents overdue invoices (True for all except Current). |

---

### 7.9 Measure Descriptions — ARMetrics

| Measure | Description |
|---------|-------------|
| **Total Open Balance** | Sum of all unpaid invoice amounts in USD. Primary AR health metric. |
| **Current Receivables Balance** | Sum of invoices not yet past due date. Represents healthy AR. |
| **Overdue Receivables Balance** | Sum of all invoices past their due date in USD. Key collection focus area. |
| **Overdue Balance Percentage** | Percentage of total AR that is overdue. Target: Below 30%. |
| **Average Days to Pay** | Average number of days customers take to pay invoices. Also known as DSO. |

---

### 7.10 Adding Synonyms

Synonyms help Copilot understand alternative terms users might use:
...
- Changes are saved automatically in Editing mode
...
The Data Agent integrates with Excel through Copilot, allowing users to query AR data and create visualizations directly in their spreadsheets. This is particularly powerful for finance teams who prefer working in Excel for analysis and reporting.

---

### 13.1 Open Copilot in Excel

1. Open **Excel** (desktop or web)
2. Click the **Copilot** button in the Home tab ribbon

<img width="1908" height="1022" alt="46  Copilot in Excel" src="https://github.com/user-attachments/assets/edac61e5-19fa-4ef5-9dde-ece39c427853" />

---

### 13.2 Access the Navigation Panel

1. The Copilot pane opens on the right side
2. Click **Open navigation panel** (hamburger menu icon) to view available agents

<img width="1903" height="1009" alt="47  Open Navigation Panel" src="https://github.com/user-attachments/assets/3e615cfb-6e6c-49ed-8ba4-36890351198c" />

---

### 13.3 Select the AR Agent

1. In the navigation panel under **Agents**, locate the **AR Agent**
2. Click to select it as the active agent

<img width="1909" height="988" alt="48  Navigate to your agent" src="https://github.com/user-attachments/assets/2618d247-8b19-4959-94c4-980e7ecca6ac" />

The panel shows your conversation history and available agents including AR Agent, Sales Intelligence Agent, and others.

---

### 13.4 Ask a Question with Visualization

Request a chart directly from the agent:

> **"Show total open balance by market classification as a donut chart"**

<img width="1904" height="962" alt="49  Asking Que to excel Agent" src="https://github.com/user-attachments/assets/f7d30dce-8020-4cac-a7ba-997274c1295e" />

The agent responds with:
- A preview of the donut chart
- Data summary (Domestic: $42.89M, International: $9.11M)
- Percentage breakdown (Domestic: 82.5%, International: 17.5%)
- Suggested follow-up actions

---

### 13.5 Add Chart to Spreadsheet

1. Click **+ Add to new sheet** to insert the visualization into Excel

<img width="1912" height="937" alt="50  Add to new sheet" src="https://github.com/user-attachments/assets/94408db8-449a-4756-8fa6-5ff29695586d" />

---

### 13.6 Chart in Excel

The donut chart is created as a native Excel chart on a new sheet:

<img width="1918" height="949" alt="51  Donut in Excel" src="https://github.com/user-attachments/assets/c26d7f9e-5530-4980-b3e2-c66d6f89aad3" />

The chart includes:
- Title: "Total Open Balance by Market Classification (USD Millions)"
- Data labels with percentages (82.5%, 17.5%)
- Legend (Domestic, International)
- Full Excel chart formatting capabilities

---

### 13.7 Additional Excel Questions

| Question | Output Type |
|----------|-------------|
| "Pull the top 10 overdue customers with their balances into a table" | Data table |
| "Create a summary of overdue balance by credit risk group" | Summary table |
| "Show open balance trend over time as a line chart" | Line chart |
| "List all customers with overdue balance over $50K sorted by amount" | Sorted data table |
| "Break down open balance by customer segment" | Bar/column chart |

---

### 13.8 Excel-Specific Benefits

| Benefit | Description |
|---------|-------------|
| **Native Charts** | Visualizations are editable Excel charts, not images |
| **Data Tables** | Raw data can be added for pivot tables and further analysis |
| **Familiar Environment** | Finance teams can work in their preferred tool |
| **Further Analysis** | Apply Excel formulas, formatting, and features to agent output |

---

### ✓ Verification

After accessing from Excel:

- AR Agent appears in the Copilot navigation panel
- Questions return data and visualizations
- Charts can be added to sheets as native Excel objects
- Data tables can be manipulated with Excel features
...
Copilot Studio allows you to create an orchestrator agent that combines multiple Fabric Data Agents into a unified experience. This enables users to ask questions across different business domains (AR and Sales) through a single conversational interface, which can then be published to Microsoft Teams.

---

### 14.1 Create Agent in Copilot Studio

1. Navigate to **Copilot Studio** (copilotstudio.microsoft.com)
2. Clcik Create a new agent 
3. Configure the agent **Details** with name and description

<img width="1914" height="955" alt="52 Adding Name and instruction" src="https://github.com/user-attachments/assets/cbdc39b7-109e-426f-bbab-24887881b308" />

**Configuration:**

| Field | Value |
|-------|-------|
| **Name** | Better Decisions Assistant |
| **Description** | Better Decisions Assistant helps you analyze business data across Accounts Receivable and Sales. Ask questions about customer balances, overdue amounts, collection priorities, sales performance, revenue trends, and pipeline analysis. This assistant routes your questions to the appropriate specialized agent for accurate, data-driven insights. |
| **Model** | GPT-4.1 (Default) |

---

### 14.2 Add Agent Instructions

Scroll down to the **Instructions** section and add orchestration instructions:

<img width="1903" height="940" alt="53  Adding Instruciton" src="https://github.com/user-attachments/assets/d0b47f2e-f06b-45a6-a7e7-c38ee58c6ebc" />
...
You are the Better Decisions Assistant — a unified business intelligence agent for Better Decisions Inc. You help executives, finance teams, and sales managers get insights across multiple business domains by routing questions to specialized agents.

## Available Knowledge Sources

You have access to two specialized data agents:

### 1. AR Agent (Accounts Receivable)
Handles questions about:
- Customer balances and overdue amounts
- Aging analysis (Current, 1-30, 31-60, 61-90, 91-120, 121-150, 151-180, 180+ days)
- Credit risk assessments (Groups A, B, C)
- Collection prioritization
- Payment performance metrics (DSO, collection rates)
- Invoice status and payment history

**Trigger phrases:** receivables, AR, overdue, aging, collections, credit risk, DSO, days to pay, invoice, payment status, who owes, past due, delinquent
...
**Trigger phrases:** sales, revenue, pipeline, opportunities, quota, forecast, deals, won, lost, sales rep, territory, product sales, bookings

## Routing Guidelines

1. **Identify the domain** — Determine if the question relates to AR/collections or Sales/revenue
2. **Route to specialist** — Direct the query to the appropriate agent
3. **Handle multi-domain questions** — For questions spanning both domains, query both agents and synthesize the response
4. **Clarify when needed** — If the domain is unclear, ask the user to clarify

## Response Guidelines

- Always identify which data source(s) you're using in your response
- Format currency as USD with appropriate abbreviations ($52M, $174K)
...
### 14.3 Add Fabric Data Agent

1. Scroll to the **Agents** section
2. Click **+ Add agent**

<img width="1911" height="934" alt="54  Add Agent" src="https://github.com/user-attachments/assets/fd4e7db5-52fb-4e5a-81b0-612385a21722" />

---

### 14.4 Select Microsoft Fabric

In the "Choose how you want to extend your agent" dialog:

1. Click **Connect to an external agent**
2. Select **Microsoft Fabric** — "Connect to a Fabric Data Agent"

<img width="1908" height="946" alt="55  External AGent" src="https://github.com/user-attachments/assets/de25617b-5b28-41d5-b517-3353966db493" />

Other connection options include Microsoft Foundry, Microsoft 365 Agents SDK, and Agent2Agent protocol.

---

### 14.5 Authenticate Connection

1. Verify the **Connection** shows your authenticated account
2. Click **Next**

<img width="1909" height="951" alt="56  Connecting to account" src="https://github.com/user-attachments/assets/3b08cbd4-2d63-4473-aed2-fc848a0eb934" />

---

### 14.6 Select the AR Agent

1. The agent list displays all available Fabric Data Agents
2. Select **AR Agent** from the list
3. Click **Next**

<img width="1907" height="950" alt="57  Adding AR Agent" src="https://github.com/user-attachments/assets/735051a4-eb21-4b62-9391-cf610c83c3dc" />

You can filter by:
- **My data** — Agents you own
- **Endorsed in your org** — Certified agents
- **Favorites** — Starred agents

---

### 14.7 Configure Agent Connection

1. Verify the **Name** (AR Agent)
2. Optionally add a **Description**
3. Confirm the **Connection** is authenticated
4. Click **Add and configure**

<img width="1904" height="946" alt="58  Add and configure" src="https://github.com/user-attachments/assets/266ced2e-241f-4a76-bee7-4e25458ef921" />

---

### 14.8 Agent Configuration Details

The agent configuration page shows connection details:

| Field | Value |
|-------|-------|
| **Name** | AR Agent |
| **Server** | Fabric data agent - Data Agent MCP Server |
| **Connection** | Authenticated user |
| **Available to** | Better Decisions Assistant |
| **Data Agent Id** | Unique artifact ID |

Ensure the **Enabled** toggle is on, then click **Publish** to save changes.

<img width="1915" height="954" alt="59  Publish CS Agent" src="https://github.com/user-attachments/assets/1a659b2a-1f34-41c9-a4e2-99dab8654d2e" />

---

### 14.9 Add Sales Intelligence Agent

Repeat steps 14.3 through 14.8 to add the **Sales Intelligence Agent**:

1. Click **+ Add agent** in the Agents section
2. Select **Microsoft Fabric** connector
3. Authenticate the connection
4. Select **Sales Intelligence Agent** from the list
5. Configure and add the agent

> **Note:** The Sales Intelligence Agent was created following the same process as the AR Agent (Steps 9-11), connecting to a Sales semantic model with its own AI configurations and agent instructions.

---

### 14.10 Connected Agents Summary

After adding both agents, the **Agents** tab shows both data agents connected and enabled:

<img width="1915" height="932" alt="60  Both Agent Available" src="https://github.com/user-attachments/assets/9a318158-17c1-47c8-9f1a-6c3ff94d6558" />

| Agent | Relationship | Trigger | Enabled |
|-------|--------------|---------|---------|
| **AR Agent** | Connected | By agent | On |
| **Sales Intelligence Agent** | Connected | By agent | On |

The Better Decisions Assistant now has access to both specialized agents and can route questions to the appropriate data source.

---

### 14.11 Publish to Teams Channel

To make the agent available in Microsoft Teams, you need to add the Teams channel.

1. From the top ribbion select **Channels**

<img width="1908" height="915" alt="61  Go to channel" src="https://github.com/user-attachments/assets/b741ba19-4eaf-41fc-8591-75187dcae1d6" />

---

### 14.12 Select Teams and Microsoft 365 Copilot

1. Navigate to the **Channels** tab
2. Under **Microsoft channels**, click **Teams and Microsoft 365 Copilot**

<img width="1907" height="862" alt="62  Select Teams" src="https://github.com/user-attachments/assets/d4b72d9f-6295-44f9-bd7a-e0b6610af93a" />

> **Note:** Because Microsoft authentication is selected, only the Teams + Microsoft 365 and SharePoint channels are available. To use other channels (Web app, Facebook, Slack, etc.), you would need to change authentication settings.

---

### 14.13 Add the Channel

The Teams and Microsoft 365 Copilot panel opens with configuration options:

<img width="1903" height="947" alt="63  Click Add chanel" src="https://github.com/user-attachments/assets/20a59b02-5b6a-4c85-a393-71db64c424d1" />

**Channel Benefits:**
- Publishing to Microsoft 365 also publishes to Teams
- Access meeting summaries and transcripts
- Pointers to open issues or unresolved questions
...
| Setting | Value |
|---------|-------|
| **Make agent available in Microsoft 365 Copilot** | ✓ Enabled |

Click **Add channel** to publish the agent to Teams.

---

### 14.14 Channel Added Confirmation

A confirmation banner appears indicating the channel was successfully added:

<img width="1922" height="985" alt="64  Channel added Confirmation" src="https://github.com/user-attachments/assets/e434f67e-0be6-4a51-8e0e-d843466d4952" />

The **Agent preview** section shows:
...
**Quick Actions:**
- **Availability options** — Configure who can access the agent
- **See agent in Microsoft 365** — Open the agent in M365 Copilot
- **See agent in Teams** — Open the agent directly in Teams

---

### ✓ Verification

After configuring Copilot Studio:

- Better Decisions Assistant is created with name, description, and instructions
- AR Agent is connected and enabled
- Sales Intelligence Agent is connected and enabled
- Teams and Microsoft 365 Copilot channel is added
- Agent is published and ready for use in Teams
...
After publishing to the Teams channel in Copilot Studio, users can interact with the Better Decisions Assistant directly within Microsoft Teams. This provides a familiar interface for employees to ask business questions without leaving their collaboration environment.

---

### 15.1 Open the Agent in Teams

1. Open **Microsoft Teams**
2. Locate the **Better Decisions Assistant** in the left sidebar (under pinned apps or recent chats)
3. Click to open the chat interface

<img width="1910" height="1013" alt="65  In teams" src="https://github.com/user-attachments/assets/0eed35cc-8191-43d6-91fa-ebb2b14fee0e" />

The agent greets you with a welcome message: "Hello, I'm Better Decisions Assistant. How can I help?"

**Teams Interface Features:**
- **Chat** tab for conversations
- **Shared** tab for shared content
- Thumbs up/down for feedback on responses
- Standard Teams message input

---

### 15.2 First-Time Authentication

When you ask your first question, Teams prompts you to authorize the Fabric Data Agent connection.

1. Type your question in the message box
2. The agent displays a **Connect to continue** dialog

<img width="1917" height="1017" alt="66  Click Allow" src="https://github.com/user-attachments/assets/5549afbf-2def-44e6-90e3-c62a5dc3f88a" />

**Connection Details:**

| Field | Value |
|-------|-------|
| **Service** | Fabric data agent |
| **Connection Type** | Data Agent MCP Server |

The dialog explains that connecting with your credentials allows the agent to retrieve information from the Fabric Data Agent. Click **Allow** to authorize the connection.

> **Note:** This authorization is remembered for future sessions. You won't need to re-authenticate each time you use the agent.

---

### 15.3 Receive Insights

After authorization, the agent queries the AR Agent and returns actionable insights:

<img width="1385" height="967" alt="67  Final Result " src="https://github.com/user-attachments/assets/e1d1e299-3912-4b99-8a53-df4a47e78205" />

**Sample Question:** "What is our total overdue balance and which customers have the highest overdue amounts?"

**Response - Overdue Receivables Insights:**

| Rank | Customer | Overdue Amount |
|------|----------|----------------|
| 1 | Wingtip Toys (Taft Heights, CA) | $173,737 |
| 2 | Wingtip Toys (Tea, SD) | $146,239 |
| 3 | Wingtip Toys (Queen Valley, AZ) | $143,022 |
| 4 | Wingtip Toys (Lost River, ID) | $141,009 |
| 5 | Tailspin Toys (Clewiston, FL) | $140,759 |

The agent also offers follow-up options: "Would you like a detailed breakdown for any of these customers or more historical context?"

---

### 15.4 Cross-Domain Queries

The Better Decisions Assistant can query **both** agents simultaneously and synthesize insights across AR and Sales data.

**Sample Question:** "Compare AR and Sales for Q4"

<img width="1465" height="964" alt="68  AR nd Sales Together" src="https://github.com/user-attachments/assets/6d429ca5-fce0-40bc-a2e1-9187c4985ad8" />

**Response - Q4 Comparison: Accounts Receivable vs. Sales:**

**Accounts Receivable (AR) – Q4 (as of 12/31/2024):**

| Metric | Value | Context |
|--------|-------|---------|
| Total Open Balance | $6.04M | |
| Overdue Balance | $3.26M | |
| Overdue Percentage | 53.86% | Well above 30% target; needs focused attention |
| DSO (Days Sales Outstanding) | -20.76 days | Negative value suggests reporting anomalies |
| Payment Collection Percentage | -1.52% | Also likely a reporting issue |

**Sales Performance – Q4 (2015):**

| Metric | Value | Context |
|--------|-------|---------|
| Total Sales | $8.0M | |
| Total Profit | $4.0M | |
| Profit Margin | 50.3% | Above company average of 49.7% |
| Units Sold | 419,200 | |
| Average Order Value | $2,367 | Down 4.8% from 2013 |
| Q4 Sales Contribution | ~25% | Of annual revenue and profit |

**Actionable Insights:**
- **AR:** High overdue percentage signals a need for improved collections and closer monitoring of customer payment behavior
- **Sales:** Strong Q4 sales and profit, but investigate the cause of declining average order value to ensure continued growth and profitability

This demonstrates the orchestrator's ability to route a single question to multiple specialized agents and deliver a unified, actionable response.

---

### 15.5 Teams-Specific Benefits

| Benefit | Description |
|---------|-------------|
| **Familiar Interface** | Users stay in their primary collaboration tool |
| **No Context Switching** | Ask business questions without opening Power BI or Excel |
| **Conversation History** | Teams preserves chat history for reference |
| **Mobile Access** | Use the agent from Teams mobile app |
| **Sharing** | Share insights with colleagues via the Shared tab |

---

### 15.6 Sample Teams Questions

| Question | Agent Used | Expected Output |
|----------|------------|-----------------|
| "What's our total overdue balance?" | AR Agent | Total overdue amount with context |
| "Which customers are in Credit Risk Group A?" | AR Agent | List of high-risk customers |
| "Show aging breakdown for Wingtip Toys" | AR Agent | Aging buckets with amounts |
| "What's our sales pipeline value?" | Sales Intelligence Agent | Pipeline summary |
| "Compare AR and Sales for Q4" | Both Agents | Cross-domain synthesis |

---

### ✓ Verification

After accessing from Teams:

- Better Decisions Assistant appears in Teams sidebar
- First-time authentication prompt appears and can be authorized
- Questions are routed to the appropriate Fabric Data Agent
- Responses include formatted insights with actionable data
- Follow-up suggestions are provided for deeper analysis
...
This playbook documented the complete process of building a **Fabric Data Agent** from a Power BI semantic model and deploying it across multiple Microsoft 365 surfaces. The solution enables natural language access to business data for users across the organization.
...
| Surface | Access Method | Use Case |
|---------|---------------|----------|
| **Power BI Copilot** | Report → Copilot pane | Analysts exploring data in context |
| **Fabric Data Agent** | Fabric portal → Test UI | Agent development and testing |
| **M365 Copilot** | microsoft365.com/copilot | Executive quick insights |
| **Excel** | Excel → Copilot → Agent | Finance teams building spreadsheets |
| **Teams** | Teams → Better Decisions Assistant | Organization-wide access |

### Key Success Factors

| Factor | Implementation |
|--------|----------------|
| **Star Schema Model** | Clean fact/dimension structure for accurate query generation |
| **Field Selection** | Exposed only relevant fields to reduce AI ambiguity |
| **AI Instructions** | Business context, terminology, and response guidelines |
| **Verified Answers** | Pre-validated responses for common business questions |
| **Field Descriptions** | Clear definitions for every exposed column and measure |
| **Orchestrator Pattern** | Single entry point routing to specialized domain agents |

### Next Steps

After deployment, consider:

1. **Monitor Usage** — Track which questions users ask most frequently
2. **Refine Instructions** — Update AI instructions based on observed response quality
3. **Add Verified Answers** — Create verified answers for recurring questions
4. **Expand Data Sources** — Connect additional semantic models as new agents
5. **Gather Feedback** — Use Teams thumbs up/down to identify improvement areas

---

**Playbook Version:** 1.0  
**Last Updated:** January 2026  
**Model:** Better Decisions Inc. — Accounts Receivable & Sales Intelligence
